...
public class MyOperationModelsProviderPlugin implements OperationModelsProviderPlugin {
	...
	private final static String basePackage = "com.xw.comm.entity.custom."; // 动态生成的Class名
	...
	private CtField createField(ApiReturnJsonPro property, CtClass ctClass)
		...
		if (property.dataType().isAssignableFrom(String.class))
			...
			value = new IntegerMemberValue(constPool,Integer.parseInt(property.example().equals("")?"0":property.example()));
		else if (property.dataType().isAssignableFrom(Long.class))
			value = new LongMemberValue(Long.parseLong(property.example().equals("")?"0":property.example()), constPool);
		else if (property.dataType().isAssignableFrom(Boolean.class))
			value = new BooleanMemberValue(Boolean.parseBoolean(property.example().equals("")?"false":property.example()), constPool);
		else if (property.dataType().isAssignableFrom(Double.class))
			value = new DoubleMemberValue(Integer.parseInt(property.example().equals("")?"0.0":property.example()), constPool);
			...
			value = new FloatMemberValue(Float.parseFloat(property.example().equals("")?"0.0":property.example()), constPool);
